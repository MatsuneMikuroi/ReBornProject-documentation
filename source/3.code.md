...
Néanmoins, les fichiers permettant de faire tourner le jeu sont tous écrits en Python. Ces derniers sont au nombre de 6 et sont divisés de façon à ce que chaque fichier ne concerne qu'un seul aspect du jeu.
### main.py
...
### AnimateSprite
Cette classe, composée de quatre méthodes en plus de celle d'initialisation, est là uniquement pour permettre l'animation ou la non-animation des différents personnages lors de leurs mouvements. La première, ```change_animation```, vient changer la partie de la planche d'images[^sheet] du personnage afin de passer à l'image suivante ou à la précédente ou bien encore de changer d'orientation.
