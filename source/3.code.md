...
```python
    def handle_input(self, handed = "right_handed"):
        ...
        self.player.move(face, sprint, dir_x, dir_y)
```
- Enfin, la fonction _run()_ est la dernière de la classe et celle qui va jouer la boucle infini de jeu tout en vérifiant les différents évèenements (rafraichissement de la map, changement de map, dialogue avec les PNJ[^pnj] ou fermeture de la fenêtre de jeu).
...
[^pnj]: Pnj est l'acronyme de "Personnage Non Joueur", ce dernier désigne avant tout toute les entités qui vont venir intéragir avec le joueur et qui sont, dans la meta du jeu, doués de conscience.