...
### game.py
Si le fichier ```main.py``` est le fichier principal, ```game.py``` et le cœur du programme. Ce dernier est composé de la classe ```Game``` qui permet au jeu son bon déroulement et son initialisation. Celle-ci est décomposée en trois méthodes distinctes qui permettent d'initialiser le jeu, récupérer les entrées du joueur et de faire tourner le jeu avec la boucle infinie de la fonction ```run()```.
...
### map.py
Le fichier ```map.py``` gère, comme son nom l'indique, de la gestion des différentes cartes disponibles dans le jeu. Ce dernier est composé des ```dataclass``` ```Portals```, qui s'occupe de stocker les informations inhérentes aux différents portails qui permettent de changer de cartes, et ```Map```, qui permet de stocker les différentes informations de ces dernières. C'est également dans ce fichier que sont stockées toutes les informations brutes des cartes. Cela inclut leur nom, leur zone, le zoom qui leur appliqué, la liste des différents portails ainsi que celle des différents PNJs.
    ...
    def register_all_map(self):
        ...
        self.register_map(
            ...
            npcs=[
                ...
                NPC("Beach", "Ichigo", nb_points=1, dialog=["Le pont est actuellement en réparation,\n "
...
### entities.py
Ce fichier sert à gérer, comme son nom l'indique, les différentes entités. Il est premier composé de la classe ```Entity``` qui hérite de la superclasse ```AnimatedSprite``` définie dans le fichier ```animation.py```. Cette dernière sert principalement à définir les différentes méthodes de déplacement du joueur ainsi que les différentes informations qui permettent les déplacements. Vient ensuite la classe ```Player``` qui hérite de la classe ```Entity``` et qui sert à initialiser le personnage qu'incarne le joueur. Enfin, vient la classe ```NPC``` qui hérite également de la classe ```Entity``` et qui sert à gérer le déplacement des différents PNJs sur les cartes.
...
### text.py
Ce fichier est celui dont la fonction est, je dirai, la moins primordiale. Il est constitué uniquement de la classe ```DialogBox``` et permet de gérer les différentes boîtes de dialogues. Or, un jeu vidéo reste un jeu vidéo même sans dialogue entre les personnages alors qu'il est difficile d'en imaginer un sans carte ou bien sans personnage à l'écran. Néanmoins, le dialogue est bien souvent une composante essentielle. Il sert, aidé de la musique et des effets sonores, à transmettre l'histoire. Même s'il est possible d'y parvenir sans, comme les excellents _Limbo_ et _Inside_ du studio _Playdead_. N'ayant pas leurs capacités, j'ai fait le choix raisonnable de faire parler les différents PNJs, et dans une future version des réponses à choix multiples seront intégrées pour le joueur afin de piloter le jeu.
...
### MapManager
Comme expliqué plus haut, la classe ```MapManager``` permet la gestion des différentes cartes du jeu. Cela commence avec les méthodes ```check_npc_collisions``` et ```check_collisions``` qui viennent vérifier les différentes collisions que le joueur et entités vont avoir entre eux et l'environnement. Dans le cas de la première, cela permet au joueur d'interagir avec les PNJs alors que la deuxième vient stopper le joueur lorsque ce dernier fonce dans une bordure ou quand un PNJ lorsque ce dernier fonce dans le joueur.
...
Vient ensuite la méthode ```tp_player``` qui permet au joueur de changer de zone.
...
### AnimateSprite
Cette classe, composée de quatre méthodes en plus de celle d'initialisation, permet l'animation ou la non-animation des différents personnages lors de leurs mouvements. La première, ```change_animation```, vient changer la partie de la planche d'images[^sheet] du personnage afin de passer à l'image suivante ou à la précédente ou bien encore de changer d'orientation.

```{figure} player.png
:alt:
Voici, par exemple, la planche d'image du joueur, chaque mouvement est divisé en trois parties.
```
...
La derbière méthode sert à identifier les personnages statiques, tous personnages devant être soit associé à la méthode avec mouvement soit à la méthode statique.
...
[^pnj]: PNJ est l'acronyme de "Personnage Non Joueur", ce dernier désigne avant tout l'entièreté des entités qui vont venir interagir avec le joueur et qui sont, dans la méta du jeu, doués de conscience.
[^sheet]: Chaque planche d'image est constituée de douze images arrangées en quatre lignes de trois colonnes. La première ligne représente le déplacement vers le bas (lorsque le personnage est face à la caméra), la seconde le déplacement vers la gauche, puis la troisième celui vers la droite et enfin la dernière le déplacement vers le haut (lorsque le personnage est dos à la caméra).


## Les différents problèmes rencontrés
Lors de l'élaboration de ce projet, je me suis retrouvé confronté à plusieurs problèmes qui peuvent, selon moi, partiellement nuire à l'expérience de jeu du joueur. L'un d'eux étant la latence qu'il y a entre le rafraîchissement de la carte et le joueur qui est visible lorsque la caméra bouge avec le joueur. Je pensais initialement que cela était dû au nombre d'éléments présents sur la carte, or des cartes de tailles d'ordres différents (plus de dix fois plus grande ou plus petite) semblent avoir la même latence à l'œil humain, cela doit donc être dû à autre chose que je n'ai pu trouver pour l'instant. Cette latence est également visible lors des changements de cartes. Vient ensuite le déplacement des PNJs qui ne peuvent se déplacer que verticalement ou horizontalement, cela est uniquement dû à la façon dont leurs déplacements sont programmés. Enfin, comme expliqué précédemment, les textes dans les boîtes de dialogues doivent être renvoyés à la ligne manuellement, car il n'y a pas d'espace d'affichage défini pour le texte.