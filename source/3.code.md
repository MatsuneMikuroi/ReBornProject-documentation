...
### DialogBox
Cette classe vient, lorsqu'une instance est générée, afficher une boite de dialogue au milieu en bas de l'écran dans laquelle est écrit le texte en police "Retro Gaming". Ce dernier peut être affiché sur plusieurs lignes mais la séparation d'une phrase en plusieurs lignes n'est actuellement pas automatisée et fonctionne grâce à un ```split``` de la chaine de caractères. La méthode ```render``` s'occupe de l'affichage, ```execute``` de vérifier si la boite de dialogue est ouverte et ```next_text``` d'afficher la prochaine partie du texte ou de fermer la boite de dialogue lorsque toutes les phrases ont été affichées.
```python
    def execute(self, dialog: list):
        if self.open:
            self.next_text()
